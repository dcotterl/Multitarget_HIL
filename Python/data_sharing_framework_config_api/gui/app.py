...
def field_definitions(selected_object):
    if isinstance(selected_object, rdma.RDMA_Configuration):
        return [("DSF version", "dsfversion", "json"), ("RDMA version", "version", "json")]
...
def modify_selected_element(tree, object_map, item_id, details_text, right_frame, root):
    ...
    for row, (label, attribute, field_type) in enumerate(field_definitions(selected_object), start=1):
        ...
        if field_type == "elements":
            ...
            widget = ttk.Combobox(panel, values=[direction.name for direction in rdma.Direction], state="readonly")
...
def parent_component(parent, default=""):
...
def show_context_menu(event, tree, object_map, details_text, right_frame, root, session):
    ...
    if isinstance(selected_object, rdma.Transfer):
        context_menu.add_command(label="add channel", command=lambda: run_tree_mutation_with_unsaved_changes(
            tree, details_text, root, lambda: add_channel_to_transfer(selected_object, refresh)
        ))
    ...
    if isinstance(selected_object, rdma.TransferGroup):
        context_menu.add_command(label="add transfer", command=lambda: run_tree_mutation_with_unsaved_changes(
            tree, details_text, root, lambda: add_transfer_to_group(selected_object, refresh)
        ))
    ...
    if isinstance(selected_object, rdma.Thread):
        context_menu.add_command(label="add group", command=lambda: run_tree_mutation_with_unsaved_changes(
            tree, details_text, root, lambda: add_group_to_thread(selected_object, refresh)
        ))
    ...
    if isinstance(selected_object, rdma.Plugin):
        context_menu.add_command(label="add thread", command=lambda: run_tree_mutation_with_unsaved_changes(
            tree, details_text, root, lambda: add_thread_to_plugin(selected_object, refresh)
        ))
    ...
    if isinstance(selected_object, rdma.RDMA_Configuration):
        context_menu.add_command(label="add plugin", command=lambda: run_tree_mutation_with_unsaved_changes(
            tree, details_text, root, lambda: add_plugin_to_configuration(selected_object, refresh)
        ))
    context_menu.tk_popup(event.x_root, event.y_root)


def add_channel_to_transfer(selected_transfer, refresh):
    protocol = parent_component(selected_transfer)
    channel_number = len(selected_transfer.channels) + 1
    selected_transfer.addChannel(rdma.Channel(name=f"Channel {channel_number}", protocol=protocol))
    refresh(selected_transfer)


def remove_channel_from_transfer(configuration, selected_channel, refresh):
    parent_transfer = find_parent(configuration, rdma.Transfer, "channels", selected_channel)
...
def remove_transfer_from_group(configuration, selected_transfer, refresh):
    parent_group = find_parent(configuration, rdma.TransferGroup, "transfers", selected_transfer)
...
def remove_group_from_thread(configuration, selected_group, refresh):
    parent_thread = find_parent(configuration, rdma.Thread, "transfer_groups", selected_group)
...
def remove_thread_from_plugin(configuration, selected_thread, refresh):
    parent_plugin = find_parent(configuration, rdma.Plugin, "threads", selected_thread)
...
def add_group_to_thread(selected_thread, refresh):
    protocol = parent_component(selected_thread)
    group_number = len(selected_thread.transfer_groups) + 1
    selected_thread.addTransferGroup(
        rdma.TransferGroup(name=f"Transfer Group {group_number}", direction=rdma.Direction.TX, protocol=protocol, transfers=[])
    )
    refresh(selected_thread)


def add_thread_to_plugin(selected_plugin, refresh):
    protocol = parent_component(selected_plugin)
    selected_plugin.addThread(rdma.Thread(processor=-2, protocol=protocol, transfer_groups=[]))
    refresh(selected_plugin)


def add_plugin_to_configuration(selected_configuration, refresh):
    plugin_number = len(selected_configuration.plugins) + 1
    selected_configuration.addPlugin(rdma.Plugin(name=f"Plugin {plugin_number}", protocol="RDMA", threads=[]))
...
def new_action(tree, file_path_label, object_map, details_text, session):
...
def main():
    ...
    file_menu.add_command(label="New", command=lambda: run_tree_mutation_with_unsaved_changes(
        tree, details_text, root, lambda: new_action(tree, file_path_label, object_map, details_text, session)
    ))
