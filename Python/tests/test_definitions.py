...
class TestDefinitionsModel(unittest.TestCase):
    ...
    def test_element(self) -> None:
        ...
        self.assertEqual(elem.getDict(), {"key": "local address", "value": "2130706433"})

        # Test __str__ uses IP formatting
        self.assertIn("127.0.0.1", str(elem))

        # Test from_dict & importFromDict
    ...
    def test_component_settings(self) -> None:
        ...
        cs.addElement("key1", "val1")
        self.assertEqual(len(cs.elements), 1)

        d_serialized = cs.getDict()
    ...
    def test_channel(self) -> None:
        ...
        rebuilt = d.Channel.from_dict(channel.getDict())
    ...
    def test_transfer(self) -> None:
        ...
        transfer.addChannel(ch2)
        self.assertEqual(len(transfer.channels), 2)

        rebuilt = d.Transfer.from_dict(transfer.getDict())
    ...
    def test_transfer_group(self) -> None:
        ...
        group.addTransfer(t1)
        self.assertEqual(len(group.transfers), 1)

        rebuilt = d.TransferGroup.from_dict(group.getDict())
    ...
    def test_thread(self) -> None:
        ...
        thread.addTransferGroup(tg)
        self.assertEqual(len(thread.transfer_groups), 1)

        rebuilt = d.Thread.from_dict(thread.getDict())
    ...
    def test_plugin(self) -> None:
        ...
        rebuilt = d.Plugin.from_dict(plugin.getDict())
